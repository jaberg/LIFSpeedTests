...
# group B: only dependent on the A state variable, only affect the B state variable
for i in range(N):
    bias=b.Jbias[i]
    encoder=[0.0]*(3*D)
    for j in range(D):
        encoder[j+D]=b.alpha[i]*b.basis[i][j]
    decoder=[0.0]*(3*D)    
    for j in range(D):
...
for i in range(1000):
    ...
    # tau_rc=0.02  (this rarely ever changes)
    # tau_ref=0.002 (this rarely ever changes)
